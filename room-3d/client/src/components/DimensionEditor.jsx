...
export default function DimensionEditor({ roomData, onChange }) {
  ...
  return (
    <div className="editor-container">
      ...
      <p className="editor-hint">Verify and adjust the extracted measurements</p>
      ...
      <div className="walls-list">
        {data.room.walls.map((wall, wi) => (
          <div key={wall.id} className={`wall-card ${wall.uncertain ? 'uncertain' : ''}`}>
            <div className="wall-header">
              <span className="wall-name">{wall.id}</span>
              {wall.uncertain && <span className="uncertain-badge">Uncertain</span>}
            </div>
            ...
            {wall.features.map((feat, fi) => (
              <div key={fi} className="feature-row">
                <span className="feature-type">{feat.type}</span>
                <label>
                  Pos
                  <input
                ...
                <button className="btn-icon" onClick={() => removeFeature(wi, fi)} title="Remove">
                  x
                </button>
              </div>
            ))}
            ...
            <div className="feature-actions">
              <button className="btn btn-secondary btn-small" onClick={() => addFeature(wi, 'door')}>
                + Door
              </button>
              <button className="btn btn-secondary btn-small" onClick={() => addFeature(wi, 'window')}>
                + Window
              </button>
            </div>
          </div>
...
function computeWallPoints(walls) {
  ...
  for (let i = 0; i < walls.length - 1; i++) {
    ...
    points.push([x, y])
    const interiorAngle = walls.length === 4 ? 90 : 90
